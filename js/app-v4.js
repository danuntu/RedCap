...
async function renderHydrants(){
  ...
  STATE.hydrants.forEach(h=>{const r=document.createElement("div"); r.className="row"; r.style.cursor="pointer"; const meta=STATE.statuses.find(s=>s.id===h.status)||{name:h.status||"—",color:"#e5e7eb"}; r.innerHTML=`<div>${h.hydrant_number??""}</div><div>${h.street_address??h.label??""}</div><div>${districtName(h.district_id)}</div><div>${h.make??""}</div><div><span style="padding:2px 6px;border-radius:999px;border:1px solid ${meta.color};color:${meta.color};font-size:12px">${meta.name}</span></div>`; r.addEventListener("click",()=>showHydrantDetail(h)); rows.appendChild(r)});
}
...
async function showHydrantDetail(h){
  ...
  $("#hydrGoFlow").onclick=()=>{ goto("flow"); $("#flowDistrict").value=h.district_id; $("#flowHydrant").value=h.hydrant_number??""; $("#flowProject").value=h.street_address??h.label??""; $("#flowInspector").focus(); };
  $("#hydrGoMI").onclick=()=>{ goto("mi"); $("#miDistrict").value=h.district_id; $("#miHydrant").value=h.hydrant_number??""; $("#miProject").value=h.street_address??h.label??""; $("#miInspector").focus(); };
  ...
  try{
    ...
    if(!records.length){ hist.innerHTML=`<div class="muted">No history recorded yet.</div>`; return }
    hist.innerHTML="";
    records.forEach(r=>{
      const row=document.createElement("div"); row.className="card"; row.style.marginBottom="8px";
      const flags=r.flags_jsonb||{};
      const review=flags.needs_review?`<div style="color:#b91c1c;font-weight:600">⚠ ${flags.needs_review.replace(/_/g," ")}</div>`:"";
      let detail="";
      if(r.inspection_type==="flow"){
        detail=`Static ${flags.static??"—"} · Residual ${flags.residual??"—"} · Pitot ${flags.pitot??"—"} · Discharge ${flags.total_discharge??flags.discharge??"—"} · AFF ${flags.aff??"—"}`;
      } else {
        detail=`Status: ${r.status??"—"}`;
      }
      row.innerHTML=`<div style="display:flex;justify-content:space-between"><b>${r.inspection_date}</b><span style="color:var(--muted)">${r.inspection_type==="flow"?"Flow Test":"M&I"}</span></div><div style="margin-top:4px">${detail}</div>${r.notes?`<div style="margin-top:4px;color:var(--muted)">${r.notes}</div>`:""}${review}`;
      hist.appendChild(row);
    });
  }catch(e){ hist.innerHTML=`<div style="color:#b91c1c">${e.message}</div>` }
}
...
function registerSW(){
  ...
  navigator.serviceWorker.getRegistrations().then(regs=>{
    regs.forEach(r=>{ if(r.active && !/\/sw-v4\.js$/.test(r.active.scriptURL)) r.unregister() });
  });
